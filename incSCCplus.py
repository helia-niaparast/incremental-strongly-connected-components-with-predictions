...
#IncSCC+ algo Fan, Hu, Tian 2017, "Incremental graph computations: Doable and undoable."
...
class IncrementalSCC:
    ...
    def realloc_ranks(self, affl: Set[int], affr: Set[int]):
        ...
        """
        affected = list(affl) + list(affr)
        # get current ranks and sort affl and affr by their current rank
        affl_sorted = sorted(affl, key=lambda x: self.rank[x])
        affr_sorted = sorted(affr, key=lambda x: self.rank[x])
        ...
        for i, node in enumerate(affl_sorted):
            self.rank[node] = all_ranks[i]
        for i, node in enumerate(affr_sorted):
            self.rank[node] = all_ranks[i + len(affl_sorted)]
    ...
    def add_new_edge(self, v: int, w: int):
        ...
        # 1. find forward and backward affected sets
        affr = self.dfs_forward(sw, self.rank[sv])
        affl = self.dfs_backward(sv, self.rank[sw])
        affected = affl.union(affr)
        # 2. build induced subgraph of Gc on affected nodes
        nodes = list(affected)
        mapping = {node: idx for idx, node in enumerate(nodes)}
        sub_adj = [[] for _ in nodes]
        for u in nodes:
            for nbr in self.Gc[u]:
                if nbr in affected:
                    sub_adj[mapping[u]].append(mapping[nbr])
        
        # 3. re-index nodes and run Tarjan to detect new SCCs
        sub_adj_mapped = sub_adj  # Already mapped
        _, new_scc = tarjan_scc(sub_adj_mapped)
        
        # 4. merge any new cycles
        comp_groups = defaultdict(list)
        for original, comp in zip(nodes, new_scc):
            comp_groups[comp].append(original)
        cycle_found = False
        for comp_nodes in comp_groups.values():
            if len(comp_nodes) > 1:
                cycle_found = True
                self.merge_sccs(set(comp_nodes))
        
        # 5. if no merge happened, just reallocate ranks
        if not cycle_found:
